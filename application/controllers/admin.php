...
class Admin extends CI_Controller 
{
	...
	public function add_new_ip()
	{
		...
		if( @$_POST['submit'] )
		{       
                    $dat= $this->admin_model->get_all_ip();
                       if(empty($dat))
			{
                           $data['inserted']=$this->admin_model->add_new_ip();	
                            ...
                            else 
                            {
                             ...
                             $data['errors'] = 'IP address already exist';
                                    $this->session->set_flashdata('errors', $data['errors']);
	...
	public function customer_management()
	{
		...
		$data['fields']=$this->admin_model->get_customer_fields();
                if($_POST)
		{
			$updated=$this->admin_model->rename_customer_fields();
			if($updated == 1)
			{
				$data['success'] = 'Field has been renamed successfully.';
				$this->session->set_flashdata('success', $data['success']);
                ...
                $data['fields']=$this->admin_model->new_customer_fields();
                if($_POST)
                {
                        $updated=$this->admin_model->new_customer_fields();
                        if($updated == 1)
                        {
                                $data['success']= 'Field has';
                        }   
                }    
	...
	public function order_managment()
	{
		...
		if($_POST)
		{
			$updated=$this->admin_model->rename_order_fields();
			if($updated == 1)
			{
				$data['success'] = 'Field has been renamed successfully.';
				$this->session->set_flashdata('success', $data['success']);
				redirect('/admin/order_managment/');
			}
