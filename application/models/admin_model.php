...
class Admin_model extends CI_Model
{
        ...
        public function get_ipaddr()
	{
        ...
        {
                $this->db->select('ip_address_xref.*');
                $data=array('ip_address',$this->input->post('ip_address'));
                $this->db->from('ip_address_xref');
                $query = $this->db->get();
	...
	{
		$data = array('username' => $this->input->post('username'),'ip_id'=>$this->input->post('ip_id'));					   
		$this->db->where('user_id', $id);
		$this->db->update('user_login', $data);	
		
		$data = array('full_name' => $this->input->post('full_name'),'gender' => $this->input->post('gender'),'phone' => $this->input->post('phone'),'privilege' =>$this->input->post('privilege'));			   
		$this->db->where('user_id', $id);
	...
	{
		$query = $this->db->get('order_fields');		
		$data = $query->result_array();
		return $data;
	}
	
	public function rename_order_fields()
	{
		foreach ($_POST['field_id'] as $k => $row)
		{
			$data = array('field_values' => $_POST['field_value'][$k]);					   
			$this->db->where('field_id', $_POST['field_id'][$k]);
	...
	public function rename_customer_fields()
	{
		foreach ($_POST['field_id'] as $k => $row)
		{			
			$data = array('field_name' => $_POST['rename_fields'][$k]);			   
			$this->db->where('field_id', $_POST['field_id'][$k]);
			$this->db->update('customer_fields', $data); 
		}
		return 1;
	}
	public function new_customer_fields()
        {
                $data = array('newname_fields' => $this->input->post('newname_fields'));	
		$this->db->insert('customer_fields', $data);
                return $this->db->insert_id();  
        }        
